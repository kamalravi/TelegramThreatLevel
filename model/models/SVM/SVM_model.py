...
def SVM_predictSample(logger, svm_predict, Sample, root_dir):
    ## Load trained model and predict
    if svm_predict == 1:
        ...
        loaded_SVMmodel = joblib.load(root_dir+'/Results/SVM/SVMmodel.pkl')
        logger.info("Predicting on the trained SVMmodel")
        y_pred = loaded_SVMmodel.predict(Sample.article)
        # save y_pred
        df = pd.DataFrame()
        
        df["article"]=Sample.article
        df["y_pred"]=y_pred

        df.to_json("/home/ravi/AdvExamplesJSON_SVMPred.json", orient='records')
        logger.info("y_pred: \n  {}".format(df))
